...
void InfogainLossLayer<Dtype>::FurtherSetUp(
  ...
  LossLayer<Dtype>::FurtherSetUp(bottom, top);
  ...
  if (bottom.size() < 3) {
    ...
    infogain_.reset(new Blob<Dtype>());
    infogain_->FromProto(blob_proto);
    infogain = infogain_.get();
  } else {
    infogain = bottom[2];
  }
...
void InfogainLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int j = 0; j < dim; ++j) {
      const Dtype prob = max(bottom_data[i * dim + j], Dtype(kLOG_THRESHOLD));
