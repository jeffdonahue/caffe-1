...
void InsertSplits(const NetParameter& param, NetParameter* param_split) {
  ...
  for (int i = 0; i < param.layers_size(); ++i) {
    ...
    const int last_top_loss =
        min(layer_param.top_loss_weight_size(), layer_param.top_size());
    for (int j = 0; j < last_top_loss; ++j) {
      const string& blob_name = layer_param.top(j);
      const pair<int, int>& top_idx = blob_name_to_last_top_idx[blob_name];
      top_idx_to_loss_weight[top_idx] = layer_param.top_loss_weight(j);
  ...
  for (int i = 0; i < param.layers_size(); ++i) {
    ...
    for (int j = 0; j < layer_param->top_size(); ++j) {
      ...
      if (split_count > 1) {
        ...
        if (loss_weight) {
          layer_param->clear_top_loss_weight();
...
void ConfigureSplitLayer(const string& layer_name, const string& blob_name,
  ...
  for (int k = 0; k < split_count; ++k) {
    ...
    if (loss_weight) {
      if (k == 0) {
        split_layer_param->add_top_loss_weight(loss_weight);
      } else {
        split_layer_param->add_top_loss_weight(0);
      }
    }
  }
}
